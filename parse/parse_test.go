...
var errAll = map[mode]bool{relaxed: true, noUnset: true, noEmpty: true, strict: true, noFail: true}
var errAllFull = map[mode]bool{relaxed: true, noUnset: true, noEmpty: true, strict: true, noFail: true}
...
var parseSkipTests = []parseTest{
	...
	// test specifically for failure modes
	{"$var not set", "${NOTSET}", "${NOTSET}", errNone},
	{"$var set to empty", "${EMPTY}", "", errEmpty},
	// restrictions for plain variables without braces
	{"gh-issue-9", "$NOTSET", "${NOTSET}", errNone},
	{"gh-issue-9", "$EMPTY", "", errEmpty},

	{"$var and $DEFAULT not set -", "${NOTSET-$ALSO_NOTSET}", "${ALSO_NOTSET}", errNone},
	{"$var and $DEFAULT not set :-", "${NOTSET:-$ALSO_NOTSET}", "${ALSO_NOTSET}", errNone},
	{"$var and $DEFAULT not set =", "${NOTSET=$ALSO_NOTSET}", "${ALSO_NOTSET}", errNone},
	{"$var and $DEFAULT not set :=", "${NOTSET:=$ALSO_NOTSET}", "${ALSO_NOTSET}", errNone},
	{"$var and $OTHER not set +", "${NOTSET+$ALSO_NOTSET}", "", errNone},
	...
	{"$var empty and $DEFAULT not set -", "${EMPTY-$NOTSET}", "", errUnset},
	{"$var empty and $DEFAULT not set :-", "${EMPTY:-$NOTSET}", "${NOTSET}", errNone},
	{"$var empty and $DEFAULT not set =", "${EMPTY=$NOTSET}", "", errEmpty},
	{"$var empty and $DEFAULT not set :=", "${EMPTY:=$NOTSET}", "${NOTSET}", errNone},
	{"$var empty and $OTHER not set +", "${EMPTY+$NOTSET}", "${NOTSET}", errNone},
	{"$var empty and $OTHER not set :+", "${EMPTY:+$NOTSET}", "${NOTSET}", errNone},
...
func TestParseNoFail(t *testing.T) {
	doTestSkipping(t, noFail)
}

func doTestSkipping(t *testing.T, m mode) {
	for _, test := range parseSkipTests {
		result, err := New(test.name, FakeEnv, restrict[m]).Parse(test.input)
		hasErr := err != nil
		if hasErr != test.hasErr[m] {
			t.Errorf("%s=(error): got\n\t%v\nexpected\n\t%v\ninput: %s\nresult: %s\nerror: %v",
